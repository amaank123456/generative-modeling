...
class UpSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        ...
        self.pixel_shuffle_op = nn.PixelShuffle(self.upscale_factor)
    ...
    def forward(self, x):
        ...
        ##################################################################
        upsampled_out = x.repeat(1, int(self.upscale_factor**2), 1, 1)
        upsampled_out = self.pixel_shuffle_op(upsampled_out)
        upsampled_out = self.conv(upsampled_out)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        ...
        self.pixel_unshuffle_op = nn.PixelUnshuffle(self.downscale_ratio)
    ...
    def forward(self, x):
        ...
        ##################################################################
        downsample_out = self.pixel_unshuffle_op(x)
        downsample_out = torch.stack(downsample_out.chunk(int(self.downscale_ratio**2), dim=1))
        downsample_out = torch.mean(downsample_out, dim=0)
        downsample_out = self.conv(downsample_out)
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            UpSampleConv2D(input_channels=n_filters, n_filters=n_filters, kernel_size=kernel_size, padding=(1, 1))
        )
        self.upsample_residual = UpSampleConv2D(input_channels=input_channels, n_filters=n_filters, kernel_size=(1, 1))
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def forward(self, x):
        ...
        ##################################################################
        temp = x.clone()
        out = self.layers(x) + self.upsample_residual(temp)   
        return out
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            DownSampleConv2D(input_channels=n_filters, n_filters=n_filters, kernel_size=kernel_size, padding=(1, 1))
        )
        self.downsample_residual = DownSampleConv2D(input_channels=input_channels, n_filters=n_filters, kernel_size=(1, 1))
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def forward(self, x):
        ...
        ##################################################################
        temp = x.clone()
        out = self.layers(x) + self.downsample_residual(temp)
        return out
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlock(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.Conv2d(in_channels=input_channels, out_channels=n_filters, kernel_size=kernel_size, stride=(1, 1), padding=(1, 1)),
            nn.ReLU(),
            nn.Conv2d(in_channels=n_filters, out_channels=n_filters, kernel_size=kernel_size, stride=(1, 1), padding=(1, 1)),
        )
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def forward_given_samples(self, z):
        ...
        ##################################################################
        out = self.dense(z)
        out = out.view(z.shape[0], 128, self.starting_image_size, self.starting_image_size)
        out = self.layers(out)
        return out
        ##################################################################
        #                          END OF YOUR CODE                      #
