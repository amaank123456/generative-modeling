...
def compute_discriminator_loss(
    ...
    loss_pt2 = torch.autograd.grad(outputs=discrim_interp, inputs = interp, grad_outputs = torch.ones_like(discrim_interp), retain_graph = True, create_graph = True)[0]
    loss_pt2 = torch.sqrt(torch.sum(loss_pt2.view(loss_pt2.shape[0],-1) ** 2, dim=1))
    loss_pt2 = lamb * torch.mean((loss_pt2 - torch.ones_like(loss_pt2)) ** 2)
    loss = loss_pt1 + loss_pt2
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
