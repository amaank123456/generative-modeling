...
def ae_loss(model, x):
    ...
    ##################################################################
    out = model.decoder(model.encoder(x))
    loss = torch.sum((out-x) ** 2)/x.shape[0]
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
...
def vae_loss(model, x, beta = 1):
    ...
    out = model.decoder(z)
    total_loss = None
    recon_loss = torch.sum((out-x) ** 2)/x.shape[0]
    kl_loss = 0.5 * torch.sum(-logvar - 1 + torch.exp(logvar) + mu ** 2)/x.shape[0]
